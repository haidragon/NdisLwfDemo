...
--*/
{
    ...
    do
    {
        ...
        Status = NdisLwfDemoRegisterDevice();
...
--*/
{
    ...
    DEBUGP(DL_TRACE, "===>NdisLwfDemo FilterPause: FilterInstance %p\n", FilterModuleContext);
...
--*/
{
    ...
    if (Status != NDIS_STATUS_SUCCESS)
    {
        ...
        //
        PWCHAR              ErrorString = L"NdisLwfDemo";
...
//
//下面添加GetNetBufferData函数的定义，用来获取网络数据的函数
...
//下面开始替换NDIS Filter框架自己的函数。
//被替换的函数有：
//FilterSendNetBufferListsComplete
//FilterSendNetBufferLists
//FilterReturnNetBufferLists
//FilterReceiveNetBufferLists
//原本NDIS Filter框架提供的这4个函数默认的代码，都没有做任何操作，没有实质性质的功能，
//这4个函数原本默认的代码，我们都不需要， 因为我们的驱动程序需要处理网络数据包，
//比如： 得到数据包里面的内容。
//
// 一个NET_BUFFER_LIST是由1个NET_BUFFER链表组成,而1个NET_BUFFER是由1个MDL链表组成。
//但网络数据包含了 多个NET_BUFFER_LIST

//Microsoft WDK 文档是这样的描述：
//In NDIS 6.0, the NET_BUFFER is the basic building block for packaging network data.
//Each NET_BUFFER structure has an MDL chain.The MDLs map the addresses of data buffers 
//to the data space that the NET_BUFFER structures specify.This data mapping is identical 
//to the MDL chains that NDIS 5.x and earlier drivers use in the NDIS_PACKET structure.
//NDIS provides functions to manipulate the MDL chain.

//Multiple NET_BUFFER structures can be attached to a NET_BUFFER_LIST structure.The NET_BUFFER 
//structures are organized as a NULL - terminated singly linked list.Only the driver that 
//originates a NET_BUFFER_LIST structure, or NDIS, should modify the linked list directly 
//to insert and delete NET_BUFFER structures.
